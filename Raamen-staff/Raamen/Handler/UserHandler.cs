...
namespace Raamen.Handler
{
    public class UserHandler
    {
        ...
        {
            return UserRepository.getAllRamen(); 
        }

        public static Raman findRamenWithId(int id)
        {
            return UserRepository.findRamenWithId(id);
        }

        public static Raman insertRamen(int meatid, string name, string broth, string price)
        {
            Raman ramen = UserFactory.createRamen(meatid, name, broth, price);

            UserRepository.insertRamen(ramen);
            return ramen;
        }

        public static void updateRamen(int id, int meatid, string name, string broth, string price)
        {
            UserRepository.updateRamen(id, meatid, name, broth, price);
        }

        public static void deleteRamen(int id)
        {
            UserRepository.deleteRamen(id);
        }
    }
}